...
# 24 Detailed Educational Paragraphs for RAG Masterclass
subtitles = [
...
dirs_to_create = [
  "templates/rag/public/audio/female",
  "templates/rag/public/audio/male",
...
async def generate_speech(text, voice, mp3_path, wav_path, copy_wav_path):
  ...
  subprocess.run(["cp", wav_path, copy_wav_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
  os.remove(mp3_path)

def get_duration(wav_path):
  cmd = ["ffprobe", "-v", "error", "-show_entries", "format=duration", "-of", "default=noprint_wrappers=1:nokey=1", wav_path]
  result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
  return round(float(result.stdout.strip()) + 0.3, 2)

async def main():
  female_durations = []
  male_durations = []

  print("Generating 24 Full Female Narration Steps...")
  for i, text in enumerate(subtitles):
    mp3 = f"templates/rag/public/audio/female/step_{i}.mp3"
    wav = f"templates/rag/public/audio/female/step_{i}.wav"
    copy_wav = f"public/assets/audio_rag_female/step_{i}.wav"
    await generate_speech(text, female_voice, mp3, wav, copy_wav)
    dur = get_duration(wav)
    female_durations.append(dur)
    print(f"  Female Step {i}: {dur}s")

  print("Generating 24 Full Male Narration Steps...")
  for i, text in enumerate(subtitles):
    mp3 = f"templates/rag/public/audio/male/step_{i}.mp3"
    wav = f"templates/rag/public/audio/male/step_{i}.wav"
    copy_wav = f"public/assets/audio_rag/step_{i}.wav"
    await generate_speech(text, male_voice, mp3, wav, copy_wav)
    dur = get_duration(wav)
    male_durations.append(dur)
    print(f"  Male Step {i}: {dur}s")

  # Write rag_durations_female.ts
  with open("templates/rag/src/rag_durations_female.ts", "w") as f:
    f.write("// Automatically generated full educational durations.\n\n")
    f.write("export const ragDurationsFemale = " + str(female_durations) + " as const;\n")

  # Write rag_durations.ts
  with open("templates/rag/src/rag_durations.ts", "w") as f:
    f.write("// Automatically generated full educational durations.\n\n")
    f.write("export const ragDurations = " + str(male_durations) + " as const;\n")

  with open("src/rag_durations.ts", "w") as f:
    f.write("// Automatically generated full educational durations.\n\n")
    f.write("export const ragDurations = " + str(male_durations) + " as const;\n")

  print("Finished! Female total:", sum(female_durations), "s | Male total:", sum(male_durations), "s")
